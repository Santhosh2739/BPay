...
import newflow_processing.DenominationsL1ProcessingNewFlow;
...
public class MobileBill_L1_ActivityNewFlowCustomerInfo extends GenericActivity implements YPCHeadlessCallback {
...
//    String response_str = null;
    ...
    //NewFlow
    public static final String KEY_DOMESTIC_RECHARGE_POJO = "KEY_DOMESTIC_RECHARGE_POJO";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.mobilebill_leg1_activity_newflow_cust_info);
...
//                }
                ...
                if (mobile_tpin_layout.getVisibility() == View.VISIBLE) {
                    if (mobile_tpin_edit.getText().toString().trim().length() == 0) {
                        Toast toast = Toast.makeText(MobileBill_L1_ActivityNewFlowCustomerInfo.this,getResources().getString(R.string.mobile_bill_L1_toast_password), Toast.LENGTH_SHORT);
                        toast.setGravity(Gravity.BOTTOM | Gravity.CENTER, 0, 400);
                        toast.show();
                        return;
                    }
...
//        domesticL1RequestPojo.setTpin(mobile_tpin_edit.getText().toString().trim());
        ...
        String domesticL1RequestPojoValue =  getIntent().getStringExtra(KEY_DOMESTIC_RECHARGE_POJO);


        DomesticL1RequestPojo domesticL1RequestPojo = new Gson().fromJson(domesticL1RequestPojoValue, DomesticL1RequestPojo.class);

        domesticL1RequestPojo.setSenderMobileNo(sender_mobile_no);
        domesticL1RequestPojo.setFirstName(name);
        domesticL1RequestPojo.setEmailId(emailID);

        CoreApplication application = (CoreApplication) getApplication();
        String uiProcessorReference = application.addUserInterfaceProcessor(new DenominationsL1ProcessingNewFlow(domesticL1RequestPojo, application, true));
