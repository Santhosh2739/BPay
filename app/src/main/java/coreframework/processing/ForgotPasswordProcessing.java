 ...
 */
public class ForgotPasswordProcessing implements UserInterfaceBackgroundProcessing {
    ...
    public ForgotPasswordProcessing(ForgotPinRequest request, CoreApplication application, boolean isPost) {
    ...
    public String captureURL() {
        ...
        this.request.setG_transType(TransType.FORGOT_PIN_REQUEST.name());
        StringBuffer buffer = new StringBuffer();
        buffer.append(TransType.FORGOT_PIN_REQUEST.getURL());


        Log.e("ForgotPassword","Data"+new Gson().toJson(this.request));

        buffer.append("?d=" + URLUTF8Encoder.encode(new Gson().toJson(this.request)));
        String send_money_url = buffer.toString();

        Log.e("ForgotPassword",""+send_money_url);
    ...
    public void processResponse(Message msg) {
        if (msg.arg1 == ServerConnection.OPERATION_SUCCESS) {
            ...
            if (response.getG_response_trans_type().equalsIgnoreCase(TransType.FORGOT_PIN_RESPONSE.name()) && response.getG_status() == 1) {
                this.response_json = network_response;
                this.success = true;
            } else if (response != null && response.getG_response_trans_type().equalsIgnoreCase(TransType.FORGOT_PIN_RESPONSE.name()) && response.getG_status() != 1) {
