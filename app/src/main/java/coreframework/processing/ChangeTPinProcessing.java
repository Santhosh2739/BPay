 ...
 */
public class ChangeTPinProcessing implements UserInterfaceBackgroundProcessing {
    ...
    public ChangeTPinProcessing(CustomerActivationRequest request, CoreApplication application, boolean isPost) {
    ...
    public String captureURL() {
        this.request.setG_transType(TransType.CHANGETPIN_REQUEST.name());
        StringBuffer buffer = new StringBuffer();
        buffer.append(TransType.CHANGETPIN_REQUEST.getURL());
    ...
    public void processResponse(Message msg) {
        if (msg.arg1 == ServerConnection.OPERATION_SUCCESS) {
            ...
            if (response != null && response.getG_response_trans_type().equalsIgnoreCase(TransType.CHANGETPIN_RESPONSE.name()) && response.getG_status() == 0) {
                ...
                this.success = true;
            } else if (response != null && response.getG_response_trans_type().equalsIgnoreCase(TransType.CHANGETPIN_RESPONSE.name()) && response.getG_status() != 0) {
    ...
    public void performUserInterfaceAndDismiss(Activity activity, ProgressDialogFrag dialogueFragment) {
        ...
        if (success) {
            Toast toast = Toast.makeText(activity, "Your Tpin has been changed successfully", Toast.LENGTH_LONG);
