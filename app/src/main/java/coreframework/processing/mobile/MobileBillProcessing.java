...
public class MobileBillProcessing implements UserInterfaceBackgroundProcessing {
    ...
    public MobileBillProcessing(MobileBillOperatorsRequest request, CoreApplication application, boolean isPost) {
    ...
    public String captureURL() {
        ...
        this.request.setG_transType(TransType.DOMESTIC_RECHARGE_IMAGE_REQUEST.name());
        StringBuffer buffer = new StringBuffer();
        buffer.append(TransType.DOMESTIC_RECHARGE_IMAGE_REQUEST.getURL());
    ...
    public void processResponse(Message msg) {
        if (msg.arg1 == ServerConnection.OPERATION_SUCCESS) {
            ...
            if (!network_response.isEmpty()) {
                ...
                if (response.getG_response_trans_type().equalsIgnoreCase(TransType.DOMESTIC_RECHARGE_IMAGE_RESPONSE.name()) && response.getG_status() == 1) {
                    this.response_json = network_response;
                    this.success = true;
                } else if (response.getG_response_trans_type().equalsIgnoreCase(TransType.DOMESTIC_RECHARGE_IMAGE_RESPONSE.name()) && response.getG_status() != 1) {
