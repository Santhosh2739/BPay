...
public class MobileBill_Denomiantions_Processing implements UserInterfaceBackgroundProcessing {
    ...
    public MobileBill_Denomiantions_Processing(DomesticL1RequestPojo request, CoreApplication application, boolean isPost) {
    ...
    public String captureURL() {
        ...
        this.request.setG_transType(TransType.DOMESTIC_RECHARGE_DENOMINATION_REQUEST.name());
        StringBuffer buffer = new StringBuffer();
        buffer.append(TransType.DOMESTIC_RECHARGE_DENOMINATION_REQUEST.getURL());
    ...
    public void processResponse(Message msg) {
        if (msg.arg1 == ServerConnection.OPERATION_SUCCESS) {
            ...
            if (!network_response.isEmpty()) {
                ...
                if (response.getG_response_trans_type().equalsIgnoreCase(TransType.DOMESTIC_RECHARGE_DENOMINATION_RESPONSE.name()) && response.getG_status() == 1) {
                    this.response_json = network_response;
                    this.success = true;
                } else if (response.getG_response_trans_type().equalsIgnoreCase(TransType.DOMESTIC_RECHARGE_DENOMINATION_RESPONSE.name()) && response.getG_status() != 1) {
    ...
    public void performUserInterfaceAndDismiss(Activity activity, ProgressDialogFrag dialogueFragment) {
        ...
        if (success) {
            Intent intent = new Intent(activity, MobileBill_L1_Activity.class);
