...
public class InvoiceDetailsProcessing implements UserInterfaceBackgroundProcessing {
    ...
    public InvoiceDetailsProcessing(GenericRequest request, CoreApplication application, boolean isPost) {
    ...
    public String captureURL() {
        ...
        this.request.setG_transType(TransType.INVOICE_DETAILS_REQUEST.name());
        StringBuffer buffer = new StringBuffer();
        buffer.append(TransType.INVOICE_DETAILS_REQUEST.getURL());
    ...
    public void processResponse(Message msg) {
        if (msg.arg1 == ServerConnection.OPERATION_SUCCESS) {
            ...
            if (!network_response.isEmpty()) {
                ...
                if (response.getG_response_trans_type().equalsIgnoreCase(TransType.INVOICE_DETAILS_RESPONSE.name()) && response.getG_status() == 1) {
                    this.response_json = network_response;
                    this.success = true;
                } else if (response.getG_response_trans_type().equalsIgnoreCase(TransType.INVOICE_DETAILS_RESPONSE.name()) && response.getG_status() != 1) {
    ...
    public void performUserInterfaceAndDismiss(Activity activity, ProgressDialogFrag dialogueFragment) {
        ...
        if (success) {
            Intent intent = new Intent(activity, InvoiceL1Activity.class);
            intent.putExtra("INVOICE_RESPONSE_DETAILS", response_json);
