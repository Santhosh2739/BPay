...
public class DenominationsL1ProcessingNewFlow implements UserInterfaceBackgroundProcessing {
    ...
    public DenominationsL1ProcessingNewFlow(DomesticL1RequestPojo request, CoreApplication application, boolean isPost) {
    ...
    public String captureURL() {
        ...
        this.request.setG_transType(TransType.NEW_DOMESTIC_RECHARGE_L1_REQUEST.name());
        StringBuffer buffer = new StringBuffer();
        buffer.append(TransType.NEW_DOMESTIC_RECHARGE_L1_REQUEST.getURL());


        Log.e("Mobile Bill", new Gson().toJson(this.request));

        buffer.append("?data=" + URLUTF8Encoder.encode(new Gson().toJson(this.request)));
        String serverURL = buffer.toString();

        Log.e("Mobile Url", serverURL);
    ...
    public void processResponse(Message msg) {
        if (msg.arg1 == ServerConnection.OPERATION_SUCCESS) {
            ...
            Log.e("Mobile Resp", network_response);

            if (!network_response.isEmpty()) {
                GenericResponse response = new Gson().fromJson(network_response, GenericResponse.class);
                if (response.getG_response_trans_type().equalsIgnoreCase(TransType.NEW_DOMESTIC_RECHARGE_L1_RESPONSE.name()) && response.getG_status() == 1) {
                    this.response_json = network_response;
                    this.success = true;
                } else if (response.getG_response_trans_type().equalsIgnoreCase(TransType.NEW_DOMESTIC_RECHARGE_L1_RESPONSE.name()) && response.getG_status() != 1) {
