...
import newflow.VirtualPrepaidMainActivityNewNewFlow;
...
public class PrepaidCardProcessingNewFlow implements UserInterfaceBackgroundProcessing {
    ...
    public PrepaidCardProcessingNewFlow(MobileBillOperatorsRequest request, CoreApplication application, boolean isPost) {
    ...
    public String captureURL() {
        ...
        this.request.setG_transType(TransType.NEW_VOCHER_IMAGE_REQUEST.name());
        StringBuffer buffer = new StringBuffer();
        buffer.append(TransType.NEW_VOCHER_IMAGE_REQUEST.getURL());
    ...
    public void processResponse(Message msg) {
        if (msg.arg1 == ServerConnection.OPERATION_SUCCESS) {
            ...
            if (!network_response.isEmpty()) {
                ...
                if (response.getG_response_trans_type().equalsIgnoreCase(TransType.NEW_VOCHER_IMAGE_RESPONSE.name()) && response.getG_status() == 1) {
                    this.response_json = network_response;
                    this.success = true;
                } else if (response.getG_response_trans_type().equalsIgnoreCase(TransType.NEW_VOCHER_IMAGE_RESPONSE.name()) && response.getG_status() != 1) {
    ...
    public void performUserInterfaceAndDismiss(Activity activity, ProgressDialogFrag dialogueFragment) {
        ...
        if (success) {
            Intent intent = new Intent(activity, VirtualPrepaidMainActivityNewNewFlow.class);
            intent.putExtra("CARD_RESPONSE", this.response_json);
            ...
            if (error_text_header.equalsIgnoreCase("Session expired")) {
                ...
                if (error_text_header.equalsIgnoreCase("Cards are not available")) {
                    Toast toast = Toast.makeText(activity, (String)activity.getResources().getString(R.string.Cards_are_not_available), Toast.LENGTH_SHORT);
                    toast.setGravity(Gravity.BOTTOM | Gravity.CENTER, 0, 400);
                    toast.show();
                } else {
